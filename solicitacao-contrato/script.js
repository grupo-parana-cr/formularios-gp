...
/* ==== BEGIN INLINE FROM ORIGINAL index.html ==== */
...
// ====== UTILIDADES ======
    ...
    function maskCNPJ(v) {
      return v.replace(/\D/g, '').replace(/(\d{2})(\d)/, '$1.$2')
        ...
        .replace(/(\d{4})(\d)/, '$1-$2').replace(/(-\d{2})\d+?$/, '$1');
    }
    ...
    function maskPhone(v) {
      ...
      // Detecta se é celular (1º dígito após DDD é 9) ou fixo
      // Celular: (XX) 9XXXX-XXXX (5x4)
      // Fixo: (XX) XXXX-XXXX (4x4)
      let formatted = '';
      
      if (digits.length > 0) formatted += `(${digits.slice(0, 2)}`;
      
      // Verifica se é celular (o 3º dígito geral = 1º após DDD é 9)
      if (digits.length >= 3) {
        const isCelular = digits[2] === '9';
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  paymentSelects.forEach(select => {
    ...
    document.addEventListener('DOMContentLoaded', function() {
      // Substituir referências de showCNPJSuccessModal por showCNPJSuccessModalV2
      window.showCNPJSuccessModal = showCNPJSuccessModalV2;
      
      // Listener para eventos de busca CNPJ
      document.addEventListener('click', function(e) {
        const btn = e.target.closest('.btn-buscar-cnpj');
        if (!btn) return;
        
        // Aqui você pode adicionar lógica de integração com sua API
        // Exemplo com tratamento de erros 400 e 404:
        ...
        //   })
        //   .then(data => showCNPJSuccessModalV2(data))
        //   .catch(err => handleAPIError(err, err.status || 500));
      });
    });
